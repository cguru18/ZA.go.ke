...
import AdminChatConsole from '../components/Chat/AdminChatConsole';
                ...
                <main className="relative z-10 max-w-7xl mx-auto p-4 sm:p-8">
                    ...
                    <div className="flex gap-6 mb-8 border-b border-white/10 relative">
                        ...
                        >
                            Overview
                            {activeTab === 'overview' && (
                                <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-0.5" style={{ backgroundColor: '#800020' }} />
                            )}
                        </button>
                        <button
                        ...
                        >
                            Support Console
                            {activeChats.reduce((a, c) => a + c.unread, 0) > 0 && (
                                <span className="px-1.5 py-0.5 rounded-full text-[9px] text-white font-bold scale-90" style={{ backgroundColor: '#800020' }}>
                                    {activeChats.reduce((a, c) => a + c.unread, 0)}
                                </span>
                            )}
                            {activeTab === 'support' && (
                                <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-0.5" style={{ backgroundColor: '#800020' }} />
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'overview' ? (
                                        ...
                                        <div className="flex-1 overflow-auto space-y-3">
                                            {activeChats.map(chat => (
                                                <motion.div key={chat.senderId}
                                                    ...
                                                    onClick={() => {
                                                        setSelectedConvId(`conv_${chat.senderId}`);
                                                        setActiveTab('support');
                                                    }}
                                                    className="p-4 rounded-full cursor-pointer transition-all duration-300 relative group border bg-white/5 border-white/10 hover:border-fuchsia-500/30 hover:bg-white/10">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-2 h-2 rounded-full bg-lilac animate-pulse" />
                                                        <h4 className="font-bold text-gray-200 text-sm">User: {chat.senderId.slice(-6)}</h4>
                                                    </div>
                            ...
                            <motion.div
                                ...
                                transition={{ duration: 0.2 }}
                            >
                                <AdminChatConsole initialConversationId={selectedConvId} />
                            </motion.div>
                        )}
                    </AnimatePresence>
                </main>
